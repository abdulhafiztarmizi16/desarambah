...
@section('title', 'Website Resmi Pemerintah Desa ' . $desa->nama_desa . ' - Laporan APBDes')
...
<div style="top:0;" class="position-absolute w-100 h-100 d-flex flex-column align-items-center justify-content-center">
    <h1 class="header-text">Laporan APBDes {{ $desa->nama_desa }}</h1>
</div>
@endsection
            ...
            <div class="nav-wrapper">
                <ul class="nav nav-pills nav-fill">
                    <li class="nav-item m-1">
                        <a class="nav-link bg-primary-500 tab {{ request('jenis') == 'laporan' ? 'active' : '' }}" href="{{ URL::current() }}?jenis=laporan&tahun={{ request('tahun') }}"><i class="fas fa-hand-holding-usd mr-2"></i>Laporan</a>
                    </li>
                    <li class="nav-item m-1">
                        <a class="nav-link bg-textprimary500 tab {{ request('jenis') == 'grafik' ? 'active' : '' }}" href="{{ URL::current() }}?jenis=grafik&tahun={{ request('tahun') }}"><i class="fas fa-chart-bar mr-2"></i>Grafik</a>
                    </li>
                </ul>
            </div>
            ...
            <table class="table table-hover table-striped table-bordered">
                ...
                <tbody>
                    @foreach ($detail_jenis_anggaran->groupBy('jenis_anggaran_id') as $key => $jenis_anggaran)
                        ...
                        @endforeach
                        @if ($key != 6)
                            <tr class="bg-primary-500 text-white font-weight-bold ">
                                <td class="text-center text-white text-uppercase" colspan="3">Jumlah {{ $jenis_anggaran[0]->jenis_anggaran->nama }}</td>
                                <td class="text-right text-white">Rp. {{ substr(number_format($anggaran_jenis, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right text-white">Rp. {{ substr(number_format($realisasi_jenis, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right text-white">Rp. {{ substr(number_format($anggaran_jenis - $realisasi_jenis, 2, ',', '.'),0,-3) }}</td>
                        ...
                        @endif
                        @if ($key == 5)
                            <tr class="bg-primary-200 text-white font-weight-bold ">
